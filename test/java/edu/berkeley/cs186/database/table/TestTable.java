...
public class TestTable {
  ...
  @Test
  @Category(StudentTest.class)
  public void testAddThenIterate() throws DatabaseException{
      Record input = TestUtils.createRecordWithAllTypes();
      RecordID[] rids = new RecordID[500];

      for (int i = 0; i < 500; i++) {
          input.getValues().get(1).setInt(i);
          rids[i] = table.addRecord(input.getValues());
      }

      Iterator<Record> recs = table.iterator();

      for (int i = 0; i < 500; i++) {
          assertTrue(recs.hasNext());
          Record rec = recs.next();
          assertTrue(rec.getValues().get(1).equals(new IntDataType(i)));
      }
  ...
  @Test(expected = DatabaseException.class)
  @Category(StudentTest.class)
  public void testDeleteInvalidCheck() throws DatabaseException {
      Record input = TestUtils.createRecordWithAllTypes();
      RecordID[] rids = new RecordID[500];
      for (int i = 0; i < 500; i++) {
          input.getValues().get(1).setInt(i);
      ...
      table.deleteRecord(rids[rand]);
      table.getRecord(rids[rand]);
      fail();
  }
  ...
  public void testInvalidUpdate() throws DatabaseException {
      ...
      table.deleteRecord(rids[rand]);
//      table.getRecord(rids[rand]);
      table.updateRecord(input.getValues(), rids[rand]);
      fail();
  }
