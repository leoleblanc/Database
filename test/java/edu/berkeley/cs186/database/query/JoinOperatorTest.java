...
public class JoinOperatorTest {
  ...
  public void testEmptyJoinWithFull() throws DatabaseException, QueryPlanException, IOException{
      TestSourceOperator leftSourceOperator = new TestSourceOperator();
      ...
      File tempDir = tempFolder.newFolder("joinTest");
      Database.Transaction transaction = new Database(tempDir.getAbsolutePath()).beginTransaction();
      JoinOperator joinOperatorSNLJ = new SNLJOperator(leftSourceOperator, rightSourceOperator, "int", "int", transaction);
      JoinOperator joinOperatorPNLJ = new PNLJOperator(leftSourceOperator, rightSourceOperator, "int", "int", transaction);
      JoinOperator joinOperatorBNLJ = new BNLJOperator(leftSourceOperator, rightSourceOperator, "int", "int", transaction);
      JoinOperator joinOperatorGrace = new GraceHashOperator(leftSourceOperator, rightSourceOperator, "int", "int", transaction);

      Iterator<Record> SNLJIter = joinOperatorSNLJ.iterator();
      Iterator<Record> PNLJIter = joinOperatorPNLJ.iterator();
      Iterator<Record> BNLJIter = joinOperatorBNLJ.iterator();
      Iterator<Record> GraceIter = joinOperatorGrace.iterator();

      assertFalse(SNLJIter.hasNext());
      assertFalse(PNLJIter.hasNext());
      assertFalse(BNLJIter.hasNext());
      assertFalse(GraceIter.hasNext());
  }

  @Test@Category(StudentTestP2.class)
  public void testEmptyJoinsAllTypes() throws DatabaseException, QueryPlanException, IOException {
      List<Integer> values = new ArrayList<Integer>();
  ...
  @Test@Category(StudentTestP2.class)
  public void testEmptyTableJoinWithSelf() throws DatabaseException, QueryPlanException, IOException {
      List<Integer> values = new ArrayList<Integer>();
      TestSourceOperator sourceOperator = TestUtils.createTestSourceOperatorWithInts(values);

      File tempDir = tempFolder.newFolder("joinTest");
      Database.Transaction transaction = new Database(tempDir.getAbsolutePath()).beginTransaction();
      JoinOperator joinOperatorSNLJ = new SNLJOperator(sourceOperator, sourceOperator, "int", "int", transaction);
      JoinOperator joinOperatorPNLJ = new PNLJOperator(sourceOperator, sourceOperator, "int", "int", transaction);
      JoinOperator joinOperatorBNLJ = new BNLJOperator(sourceOperator, sourceOperator, "int", "int", transaction);
      JoinOperator joinOperatorGrace = new GraceHashOperator(sourceOperator, sourceOperator, "int", "int", transaction);
    ...
    @Test(expected=NullPointerException.class)@Category(StudentTestP2.class)
    public void testNullFieldInput () throws QueryPlanException, DatabaseException, IOException {
        TestSourceOperator sourceOperator = new TestSourceOperator();
        File tempDir = tempFolder.newFolder("joinTest");
        Database.Transaction transaction = new Database(tempDir.getAbsolutePath()).beginTransaction();
        new SNLJOperator(sourceOperator, sourceOperator, null, "int", transaction);
    }
}
