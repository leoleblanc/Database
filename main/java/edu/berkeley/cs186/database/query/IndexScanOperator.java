...
public class IndexScanOperator extends QueryOperator {
  ...
  public int estimateIOCost() throws QueryPlanException {
    ...
    float redFact;
    try {
        numPages = this.transaction.getNumIndexPages(tableName, columnName);
        numTuples = this.transaction.getNumRecords(tableName);
        redFact = this.transaction.getStats(tableName).getReductionFactor(columnIndex, predicate, value);

    } catch (DatabaseException d) {
        throw new QueryPlanException(d);
    }
    return (int) Math.ceil((numPages + numTuples) * redFact);
  }
