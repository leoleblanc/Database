...
public class IntHistogram implements Histogram<Integer>{
  ...
  public float computeReductionFactor(PredicateOperator predicate,
    ...
    boolean ltm = val < min;
    boolean mtm = val > max;
    float numKeys = getNumDistinct(); //the number of distinct keys, what's it for tho
    if (numKeys == 0) {
        return 0;
    }

    float predVals = (float) 0.0;
    if (predicate.toString().equals("EQUALS")) {
        if (ltm || mtm) {
            predVals = 0;
        } else {
            predVals = (float) 1 / numKeys;
        }
    } else if (predicate.toString().equals("NOT_EQUALS")) {
        if (ltm || mtm) {
            predVals = 1;
        } else {
            predVals = 1 - ((float) 1 / numKeys);
        }
    } else if (predicate.toString().equals("LESS_THAN")) {
        if (val < min) {
            predVals = 0;
        } else {
            predVals = (float) (val - min) / (max - min);
        }
    } else if (predicate.toString().equals("LESS_THAN_EQUALS")) {
        if (val < min) {
            predVals = 0;
        } else {
            predVals = ((float) (val - min) / (max - min) + (float) 1 / numKeys);
        }
    } else if (predicate.toString().equals("GREATER_THAN")) {
        if (val > max) {
            predVals = 0;
        } else {
            predVals = (float) (max - val) / (max - min);
        }
    } else if (predicate.toString().equals("GREATER_THAN_EQUALS")) {
        if (val > max) {
            predVals = 0;
        } else {
            predVals = ((float) (max - val) / (max - min) + (float) 1 / numKeys);
        }
    } else {
        System.out.println("WHAT IS THIS");
    }
    reductionFactor = predVals;
