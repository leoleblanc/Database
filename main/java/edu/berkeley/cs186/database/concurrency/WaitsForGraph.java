...
public class WaitsForGraph {
  ...
  protected boolean edgeCausesCycle(long from, long to) {
    //TODO: Implement Me!!
    boolean exists = edgeExists(from, to);
    if (!exists) {
        addEdge(from, to);
    }

    boolean result = edgeCausesCycleHelper(from, to, from, true);
  ...
  private boolean edgeCausesCycleHelper(long from, long to, long goal, boolean first) {
      if ((from == goal) && !first) {
          return true;
      }
      boolean cycle = false;
      for (Long neighbor: graph.get(from)) {
          cycle = edgeCausesCycleHelper(neighbor, to, goal, false);
          if (cycle) {
              break;
          }
      }
